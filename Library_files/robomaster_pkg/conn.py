...
def scan_robot_ip(user_sn=None, timeout=5.0):
    """ 扫描机器人的IP地址
...
def scan_robot_ip_list(timeout=3.0):
    ...
    while True:
        ...
        if ip[0] not in ip_list:
            ip_list.append(ip[0])
            sn = str(data[:-1].decode(encoding='utf-8')) #! edit
            sn_list.append(sn) #! edit
...
class SdkConnection(BaseConnection):
    ...
    #! Edit: no need to rescan robot ip we want to connect
    def request_connection2(self, sdk_host, conn_type=None, proto_type=None, ip=None, sn=None):
        if conn_type is None:
            logger.error("Not Specific conn_type!")
        logger.info("CONN TYPE is {0}".format(conn_type))
        local_addr = None
        ...
        if conn_type is CONNECTION_WIFI_AP:
            ...
            if config.ROBOT_IP_STR:
                ...
                remote_ip = ip #scan_robot_ip(sn) #! Edit
                if not remote_ip:
                    return False, None, None
            local_addr = (local_ip, proto._port)
            remote_addr = (remote_ip, config.ROBOT_DEVICE_PORT)
