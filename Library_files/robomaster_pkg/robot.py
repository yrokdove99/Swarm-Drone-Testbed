...
class Robot(RobotBase):
    ...
    def __init__(self, cli=None): #! edit
    ...
    def initialize2(self, conn_type=config.DEFAULT_CONN_TYPE, proto_type=config.DEFAULT_PROTO_TYPE, ip=None, sn=None):
        """ 初始化机器人
        ...
        if not self._client:
            logger.info("Robot: try to connection robot.")
            conn1 = self._wait_for_connection2(conn_type, proto_type, ip, sn)
            if conn1:
                logger.info("Robot: initialized with {0}".format(conn1))
                self._client = client.Client(9, 6, conn1)
            else:
    ...
    def _wait_for_connection2(self, conn_type, proto_type, ip, sn=None):
        result, local_addr, remote_addr = self._sdk_conn.request_connection2(self._sdk_host, conn_type, proto_type, ip, sn)
        if not result:
            logger.error("Robot: Connection Failed, Please Check Hareware Connections!!! "
                         "conn_type {0}, host {1}, target {2}.".format(conn_type, local_addr, remote_addr))
            return None
